...
const KelolaDiskon = () => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (editingId) {
        ...
        // Kirim Notifikasi Diskon Baru
        const newNotif = {
          message: `Diskon Baru: ${values.name} (${values.percentage}%) Telah Tersedia!`,
  ...
  const columns = [
    ...
    { 
      ...
      key: 'type',
      render: (type) => {
        let color = type === 'Student' ? 'blue' : type === 'Professional' ? 'purple' : 'green';
    ...
    <div style={{ padding: '20px' }}>
      ...
      <Modal title={editingId ? "Edit Diskon" : "Tambah Diskon"} open={isModalOpen} onCancel={() => setIsModalOpen(false)} onOk={() => form.submit()}>
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          <Form.Item name="validUntil" label="Berlaku Sampai" rules={[{ required: true }]}><DatePicker style={{ width: '100%' }} /></Form.Item>
        </Form>
      </Modal>
    </div>
  );
