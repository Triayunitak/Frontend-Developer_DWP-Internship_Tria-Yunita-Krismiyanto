...
import { 
  ...
  UserOutlined, BellOutlined, LogoutOutlined, ArrowRightOutlined 
...
const PackageDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        message.error("Gagal terhubung ke database");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'var(--hitam)' }}><Spin size="large" tip="Memuat Detail..." /></div>;
  if (!pkg) return null;
